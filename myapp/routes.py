...
from myapp.forms import EmptyForm, LoginForm, RegistrationForm, EditProfileForm, EmptyForm, PostForm, ResetPasswordRequestForm, ResetPasswordForm
...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f"You - {current_user} - are now following {user}")
        return redirect(url_for("user", username=username))
    else:
        return redirect(url_for('index'))
...
def unfollow(username):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f"You - {current_user} - are now unfollowing {user}")
        return redirect(url_for("user", username=username))
    else:
        return redirect(url_for('index'))
